...
async function main() {
  ...
  console.log("Finding VS PEANUT CANDY product...");
  const product = await client.fetch('*[_type == "product" && name == "VS PEANUT CANDY 200G X 30"][0]');
  if (!product) {
    console.error("Could not find product matching VS PEANUT CANDY 200G X 30!");
    return;
  }
  ...
  console.log("Successfully moved 'VS PEANUT CANDY 200G X 30' to VISWAS SNACKS!");
}
